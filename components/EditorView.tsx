...
import { CloseIcon, PublishIcon, SunIcon, MoonIcon, SparklesIcon } from './Icons';
...
const EditorHeader: React.FC<{ projectName: string; onPublish: () => void; theme: Theme; onThemeChange: (theme: Theme) => void }> = ({ projectName, onPublish, theme, onThemeChange }) => (
...
const ErrorBanner: React.FC<{ message: string; onFix: () => void; onClose: () => void }> = ({ message, onFix, onClose }) => (
    <div className="flex items-center justify-between p-3 bg-red-800/20 border-b border-red-500/30 text-red-200 text-sm flex-shrink-0 animate-fadeIn">
        <div className="flex items-center gap-3 overflow-hidden">
            ...
            <p className="font-medium truncate flex-1">Erro na visualização: <span className="font-normal opacity-90">{message}</span></p>
        </div>
...
export const EditorView: React.FC<EditorViewProps> = ({ files, activeFile, projectName, theme, onThemeChange, onFileSelect, onFileClose, onPublish, codeError, onFixCode, onClearError, onError }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-var-bg-subtle">
      <EditorHeader projectName={projectName} onPublish={onPublish} theme={theme} onThemeChange={onThemeChange} />
      ...
      <div className="flex-grow overflow-auto bg-var-bg-subtle">
        {viewMode === 'code' ? (
          ...
          <CodePreview files={files} onError={onError} theme={theme} />
        )}
      </div>
    </div>
  );
