    ...
    <div className="bg-[#09090b] flex h-full border-r border-[#27272a] select-none w-full">
        ...
        <div className="flex-1 bg-[#121214] flex flex-col h-full overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto py-2 custom-scrollbar">
                {activeTab === 'files' ? (
                    ...
                    <div className="p-3 space-y-3">
                         {[
                            { icon: <GithubIcon />, title: "GitHub", desc: "Sync repos", onClick: onOpenGithubImport, color: "hover:border-gray-500" },
                            { icon: <SupabaseIcon />, title: "Supabase", desc: "DB & Auth", onClick: onOpenSupabaseAdmin, color: "hover:border-green-500/50" },
                            { icon: <StripeIcon />, title: "Stripe", desc: "Payments", onClick: onOpenStripeModal, color: "hover:border-indigo-500/50" },
                            { icon: <DatabaseIcon />, title: "Neon", desc: "Postgres", onClick: onOpenNeonModal, color: "hover:border-emerald-500/50" },
                        ].map((item, idx) => (
                            <div key={idx} className={`bg-[#18181b] p-3 rounded-lg border border-[#27272a] transition-colors group cursor-pointer ${item.color}`} onClick={item.onClick}>
            ...
            <div className="p-3 border-t border-[#27272a]">
                <button onClick={onDownload} className="w-full flex items-center justify-center gap-2 bg-[#18181b] hover:bg-[#27272a] text-gray-300 hover:text-white text-xs py-2 rounded border border-[#27272a] transition-colors">
      ...
      <div className="p-3 space-y-3 flex flex-col h-full">
        <div className="flex-grow overflow-y-auto space-y-2 pr-1 custom-scrollbar">
            ...
            <div key={index} className="flex items-center gap-2 group">
                <input type="text" placeholder="KEY" value={key} onChange={(e) => handleChange(index, 'key', e.target.value)} className="w-1/3 p-1.5 bg-[#18181b] border border-[#27272a] rounded text-gray-300 text-[11px] font-mono focus:outline-none" />
                <input type="password" placeholder="VALUE" value={value} onChange={(e) => handleChange(index, 'value', e.target.value)} className="w-full p-1.5 bg-[#18181b] border border-[#27272a] rounded text-gray-300 text-[11px] font-mono focus:outline-none" />
                <button onClick={() => handleRemove(index)} className="p-1 text-gray-500 hover:text-red-400 opacity-0 group-hover:opacity-100"><TrashIcon className="w-3 h-3" /></button>
            </div>
            ))}
        </div>
        <div className="flex-shrink-0 pt-3 border-t border-[#27272a] space-y-2">
            <button onClick={handleAdd} className="w-full text-[11px] py-1.5 border border-dashed border-[#27272a] rounded text-gray-400 hover:text-white">+ Add Variable</button>
            <button onClick={() => { onSave(localVars.reduce((acc, [k,v]) => (k ? (acc[k]=v,acc) : acc), {} as any)); setHasChanges(false); }} disabled={!hasChanges} className="w-full bg-white text-black text-[11px] font-bold py-1.5 rounded disabled:opacity-50">Save Changes</button>
        </div>
      </div>
    );
};
